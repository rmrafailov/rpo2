...
class RPOTrainer(BaseTrainer):
    ...
    def step(
        ...
        bs = None   # self.config.batch_size
        ...
        with torch.no_grad():
            all_logprobs, _, values, all_masks = self.batched_forward_pass(self.model, queries, responses, model_inputs)
            ...
            if self.is_peft_model and hasattr(
                ...
                all_ref_logprobs, _, _, _ = self.batched_forward_pass(self.ref_model, queries, responses, model_inputs)

        # upcast to float32 to avoid dataset issues
        mini_batch_dict = {
            "queries": [queries[i:i+self.K] for i in range(0, len(queries), self.K)],
            "responses": [responses[i:i+self.K] for i in range(0, len(responses), self.K)],
            "scores": [scores[i:i+self.K] for i in range(0, len(scores), self.K)]
        }
        model_inputs = {k: torch.split(v, self.K) for k, v in model_inputs.items()}

        def collator(data):
            return_dict = dict()
            for key in data[0]:
                if key in ["queries", "responses"]:
                    return_dict[key] = [item for d in data for item in d[key]]
                else:
                    return_dict[key] = torch.stack([item for d in data for item in d[key]]).to(self.current_device)
            return return_dict
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for i, batch in enumerate(mini_batch_dataloader):
                with self.accelerator.accumulate(self.model):
                    ...
                    logprobs, _, _, mask = self.batched_forward_pass(
                        self.model,
                        batch["queries"],
                        batch["responses"],
                        model_inputs,
                    ...
                    with torch.no_grad():
                        ref_logprobs, _, _, ref_mask = self.batched_forward_pass(
                            self.ref_model,
                            batch["queries"],
                            batch["responses"],
                            model_inputs,
    ...
    def batched_forward_pass(
        ...
        for i in range(int(bs / fbs)):
            ...
            for j in range(fbs):
                ...
                masks[j, :start] = 0
                masks[j, end:] = 0
                print("Attention mask", attention_mask[j, :])
                print("Mask", masks[j, :])
    ...
    def train_minibatch(
        ...
        """
        loss, train_stats = self.loss(reflogprobs, logprobs, scores, ref_mask, mask)
    ...
    def loss(
        ...
        print("Loss start reflogprobs", reflogprobs)
        print("Loss start logprobs", logprobs)
        print("Loss start scores", scores)
        print("Loss start mask", mask)

        reflogprobs = torch.sum(reflogprobs * refmask, axis=-1)
        logprobs = torch.sum(logprobs * mask, axis=-1)

        reflogprobs = torch.stack(torch.split(reflogprobs, self.K, 0), 0)
        logprobs = torch.stack(torch.split(logprobs, self.K, 0), 0)
        scores = torch.stack(torch.split(scores, self.K, 0), 0)
        mask = torch.stack(torch.split(mask, self.K, 0), 0)

        ratios = 0.1 * (logprobs - reflogprobs)
        w = torch.softmax(scores, axis=-1)

        print("Beta", self.kl_ctl.value)
        print(ratios.shape)
        print(scores.shape)
        print(w.shape)
        print(torch.sum(w * ratios, -1).shape)
        print(torch.logsumexp(ratios, axis = -1).shape)
        loss = -torch.mean(torch.sum(w * ratios, -1)- torch.logsumexp(ratios, axis = -1))
        #loss = -torch.mean(torch.sum(w * torch.nn.LogSoftmax(dim=-1)(ratios), -1))
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        stats = {
            ...
            "rpo/mean_scores": mean_scores,
            "rpo/std_scores": std_scores,
        }
    ...
    def log_stats(
        ...
        print("In log statst", len(batch["query"]), len(["response"]))
